...
export default class XBrokerClient {
  ...
  resubscribe(): void {
    for(const channel: string in this.subscriptions) {
      ...
      this.resubscribeCommand(ce.command.cmd, ce.command.args, callback, ce.listener);
    }
  ...
  resubscribeCommand(cmd: string, args: Array<string|number>, callback: Callback, listener: ?Callback): string {
    const command = {tag: this.seq.toString(), cmd: cmd, args: args};
    this.seq++;
  ...
  command(cmd: string, args: Array<string|number>, callback: Callback, listener: ?Callback): string {
    const command = {tag: this.seq.toString(), cmd: cmd, args: args};
    this.seq++;
    const commandEntry: CommandEntry = {
      command,
  ...
  commandAsync(cmd: string, args: Array<string|number>, listener: ?Callback): Promise<mixed> {
    const self = this;
    return new Promise(function(resolve, reject) {
      ...
      self.command(cmd, args, callback, listener);
    });
  }
