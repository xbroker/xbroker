...
import XBrokerClient from '../client/XBrokerClient'
...
test("client.command", function(done) {
  client.command("exists", ["nosuchkey"], 
    (error, result) => {
...
test("illegal command", function(done) {
  client.command("eexists", ["nosuchkey"], 
    (error, result) => {
      expect(error).toBeUndefined()
      expect(result).toBeDefined()
      expect(result).not.toBeNull()
...
test("client.commandAsync - using async", async function() {
  const result = await client.commandAsync("keys", ["*"])
  expect(result).toEqual(expect.arrayContaining([]))
})

test("client.commandAsync - using promise", function() {
  return client.commandAsync("keys", ["*"])
  .then((result) => {
    expect(result).toEqual(expect.arrayContaining([]))
  })
})
...
test("subscribe/unsubscribe - using async", async function() {
  var result = await client.commandAsync("subscribe", ["ABC"])
  expect(result).toEqual(expect.arrayContaining([]))

  result = await client.commandAsync("unsubscribe", ["ABC"])
  expect(result).toEqual(expect.arrayContaining([]))
})

test("psubscribe/punsubscribe - using async", async function() {
  var result = await client.commandAsync("psubscribe", ["ABC1"])
  expect(result).toEqual(expect.arrayContaining([]))

  result = await client.commandAsync("punsubscribe", ["ABC1"])
  expect(result).toEqual(expect.arrayContaining([]))
})

test("subscribe twice", async function() {
  var result = await client.commandAsync("subscribe", ["ABC2"])
  expect(result).toEqual(expect.arrayContaining([]))

  try {
    result = await client.commandAsync("subscribe", ["ABC2"])
  } catch(error) {
...
test("psubscribe twice", async function() {
  var result = await client.commandAsync("psubscribe", ["ABC3"])
  expect(result).toEqual(expect.arrayContaining([]))

  try {
    result = await client.commandAsync("psubscribe", ["ABC3"])
  } catch(error) {
    expect(error.status).toBe('error')
